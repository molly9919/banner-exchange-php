...
<div class="container">
    ...
    <div class="card">
        ...
        <textarea readonly><iframe src="<?= htmlspecialchars(($settings['app_url'] ?? '') ?: ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost')) ) ?>/serve.php?size=300x300&amp;user=<?= (int) $userId ?>" width="300" height="300" frameborder="0" scrolling="no"></iframe></textarea>
        <p class="small">WordPress HTML widget alternative (JS embed):</p>
        <textarea readonly><script async src="<?= htmlspecialchars(($settings['app_url'] ?? '') ?: ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost')) ) ?>/adjs.php?size=468x60&amp;user=<?= (int) $userId ?>"></script></textarea>
    </div>
